...
class SubProjectViewController: UIViewController {
    ...
    var getDepatmentModel = [SubprojectModel]()
...
//        }
    ...
    @IBAction func createAction(_ sender: Any) {
        let vc = adminStoryboard.instantiateViewController(withIdentifier: "CreateSubprojectViewController") as! CreateSubprojectViewController
...
extension SubProjectViewController : UITableViewDataSource,UITableViewDelegate, customCellDropdownDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
       ...
       // cell.viewMoreHeight.constant = 0
        cell.lblProductId.text = self.getDepatmentModel[indexPath.row].subProjectName
        //cell.lblDesc.text = "Subproject Description"
        cell.lblDescDetail.text = self.getDepatmentModel[indexPath.row].subProjectDesc
        //cell.lblSelectDesc.text = "Cost Center"
        let costCenterDesc = self.getCostCenter.filter({ (result) -> Bool in
            return result.id == self.getDepatmentModel[indexPath.row].projectId
        })
        if costCenterDesc.count > 0{
        cell.lblSelectDescDetail.text = "\(costCenterDesc.first!.projectName!) - \(costCenterDesc.first!.projectDesc!)"
        }
    ...
    func deleteCostCenter(deleteCenter : SubprojectModel){
//        showLoader()
//        networking.setAuthorizationHeader(token: DefaultsManager.shared.accesstoken ?? "")
//        networking.delete(DELETE_SUBPROJECT + "\(deleteCenter.id!)", parameters: nil) { (result) in
//            hideLoader()
//            do{
//            switch result {
//            case .success(let response):
