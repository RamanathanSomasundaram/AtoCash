//
//  EmployeeTypeViewController.swift
//  AtoCash
//
...
class EmployeeTypeViewController: UIViewController {

    @IBOutlet weak var tblView: UITableView!
    var getCostCenter = [EmploymentType]()
    ...
    func getAllCostCenters(){
        ...
        EmploymentTypesAPI.apiEmploymentTypesGet { (result, error) in
            hideLoader()
            if error == nil {
    ...
    @IBAction func createAction(_ sender: Any) {
        let vc = adminStoryboard.instantiateViewController(withIdentifier: "CreateEmployeeTypeViewController") as! CreateEmployeeTypeViewController
...
extension EmployeeTypeViewController : UITableViewDataSource,UITableViewDelegate, customCellButtonActionDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.lblProductId.text = " " + getCostCenter[indexPath.row].empJobTypeCode + " "
        cell.lblDetailDesc.text = getCostCenter[indexPath.row].empJobTypeDesc
    ...
    func editActionTag(_ sender: Int) {
        ...
        let vc = adminStoryboard.instantiateViewController(withIdentifier: "CreateEmployeeTypeViewController") as! CreateEmployeeTypeViewController
        vc.editEmployeeModel = getCostCenter[sender]
        self.navigationController?.pushViewController(vc, animated: true)
    ...
    func deleteCostCenter(deleteCenter : EmploymentType){
        showLoader()
        EmploymentTypesAPI.apiEmploymentTypesIdDelete(_id: deleteCenter._id!) { (result, error) in
            hideLoader()
            if error == nil {
                Loaf("Employment deleted successfully", state: .success, location: .top, sender: self).show(.short, completionHandler: nil)
                DispatchQueue.main.async {
                    self.getAllCostCenters()
                }
            }
