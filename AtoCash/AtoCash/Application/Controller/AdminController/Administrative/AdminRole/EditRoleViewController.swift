...
class EditRoleViewController: UIViewController {

    @IBOutlet weak var txtRoleName: InputTextFieldDesign!
    @IBOutlet weak var createBtn: UIButton!
    var editAdminRole : AdminRoleModel?
    override func viewDidLoad() {
        ...
        if editAdminRole == nil {
            ...
            self.txtRoleName.text = editAdminRole?.name
        }
...
//        networking.setAuthorizationHeader(token: DefaultsManager.shared.accesstoken ?? "")
//            networking.put(PUT_ADMIN_ROLE + "/\(self.editAdminRole!.id!)", parameterType: .json, parameters: params) { (result) in
//            hideLoader()
//            do{
//            switch result {
//            case .success(let response):
...
extension EditRoleViewController : UITextFieldDelegate{
