//
//  ApprovalRoleMapViewController.swift
//  AtoCash
//
...
class ApprovalRoleMapViewController: UIViewController {

    @IBOutlet weak var tblView: UITableView!
    var getDepatmentModel = [ApprovalRoleMapDTO]()
    ...
    @IBAction func createAction(_ sender: Any) {
        let vc = adminStoryboard.instantiateViewController(withIdentifier: "CreateApprovalRoleMapViewController") as! CreateApprovalRoleMapViewController
        vc.editApprovalRoleMap = nil
...
extension ApprovalRoleMapViewController : UITableViewDataSource,UITableViewDelegate, customCellDropdownDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.lblProductId.text =  " " + self.getDepatmentModel[indexPath.row].approvalGroup! + " "
        cell.lblDescDetail.text = self.getDepatmentModel[indexPath.row].role
        cell.lblSelectDescDetail.text = "\(self.getDepatmentModel[indexPath.row].approvalLevel!)"
    ...
    func editActionTag(_ sender: Int) {
        let vc = adminStoryboard.instantiateViewController(withIdentifier: "CreateApprovalRoleMapViewController") as! CreateApprovalRoleMapViewController
        vc.editApprovalRoleMap = self.getDepatmentModel[sender]
        self.navigationController?.pushViewController(vc, animated: true)
    ...
    func deleteCostCenter(deleteCenter : ApprovalRoleMapDTO){
        
        showLoader()
        ApprovalRoleMapsAPI.apiApprovalRoleMapsIdDelete(_id: deleteCenter._id!) { (result, error) in
            hideLoader()
            if error == nil {
                Loaf("Approval Role map deleted successfully", state: .success, location: .top, sender: self).show(.short, completionHandler: nil)
                DispatchQueue.main.async {
                    self.getRoleMaps()
                }
            }
...
//        networking.delete(APPROVAL_ROLE_MAP + "\(deleteCenter.id!)", parameters: nil) { (result) in
//            hideLoader()
//            do{
//            switch result {
